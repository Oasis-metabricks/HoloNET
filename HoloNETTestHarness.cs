...
namespace NextGenSoftware.Holochain.HoloNET.Client.TestHarness
{
    public class HoloNETTestHarness
    {
        ...
        static async Task Main(string[] args)
        {
            ...
            // Run the enhanced demo that shows HoloNET in action
            await TestHoloNETClientAsync(TestToRun.EnhancedDemo);
        ...
        public static async Task TestHoloNETClientAsync(TestToRun testToRun)
        {
            ...
            switch (testToRun)
            {
                case TestToRun.TestHarnessGuide:
                case TestToRun.SimpleTest:
                case TestToRun.EnhancedDemo:
                    // Test harness guide, simple test and enhanced demo don't need any special configuration
                    break;
            ...
            if (_testToRun != TestToRun.SaveLoadOASISEntryUsingSingleHoloNETAuditEntryBaseClass && _testToRun != TestToRun.SimpleTest && _testToRun != TestToRun.EnhancedDemo && _testToRun != TestToRun.TestHarnessGuide)
            {
                switch (_testToRun)
                {
                    case TestToRun.SimpleTest:
                        // Simple test doesn't need admin client
                        break;
            ...
            switch (testToRun)
            {
                case TestToRun.TestHarnessGuide:
                    {
                        CLIEngine.ShowMessage("*** HOLONET TEST HARNESS & DASHBOARD GUIDE ***", _testHeadingColour);
                        await HoloNETTestHarnessGuide.ShowAvailableTestHarnesses();
                    }
                    break;
                    
                case TestToRun.SimpleTest:
                    {
                        CLIEngine.ShowMessage("*** HOLONET SIMPLE TEST - NO CONDUCTOR REQUIRED ***", _testHeadingColour);
                ...
                case TestToRun.EnhancedDemo:
                    {
                        CLIEngine.ShowMessage("*** HOLONET ENHANCED DEMO - INTERACTIVE SHOWCASE ***", _testHeadingColour);
                        await EnhancedDemo.RunEnhancedDemo();
                    }
                    break;
